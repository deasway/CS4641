 ...
 * find optimal weights to a neural network that is classifying potential customers as
 * "will subscribe to a term deposit" and "will not subscribe to a term deposit"
 *
 * @author Hannah Lau, Alan Li
 * @version 1.0
 */
public class NeuralNetOptimization {
    ...
    public static void main(String[] args) {
        ...
        for(int i = 0; i < oa.length; i++) {
            ...
            for (int k = 0; k < 36169; k++) {
                networks[i].setInputValues(instances[k].getData());
                networks[i].run();

                predicted = Double.parseDouble(instances[k].getLabel().toString());
                actual = Double.parseDouble(networks[i].getOutputValues().toString());
            ...
            results +=  "\nResults for  " + oaNames[i] + "testing testing: \nCorrectly classified " + correct_test + " instances." +
                    "\nIncorrectly classified " + incorrect_test + " instances.\nPercent correctly classified: "
                    + df.format(correct_test/(correct_test+incorrect_test)*100) + "%\nTraining time: " + df.format(trainingTime)
                    + " seconds\nTesting time: " + df.format(testingTime) + " seconds\n";
            results +=  "\nResults for  " + oaNames[i] + "training set: \nCorrectly classified " + correct_train + " instances." +
    ...
    private static void train(OptimizationAlgorithm oa, BackPropagationNetwork network, String oaName) {
        ...
        for(int i = 0; i < trainingIterations; i++) {
            ...
            for(int k = 0; k <test_instances.length; k++) {
                network.setInputValues(test_instances[k].getData());
                network.run();

                Instance output = test_instances[k].getLabel(), example = new Instance(network.getOutputValues());
                example.setLabel(new Instance(Double.parseDouble(network.getOutputValues().toString())));
                test_error += measure.value(output, example);

            }

            System.out.println(df.format(error) + "," + df.format(test_error));
        }
    }
